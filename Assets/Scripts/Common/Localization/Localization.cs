...
static public class Localization
{
    ...
    static public Dictionary<string, Font> mFontDictionary = new Dictionary<string, Font>();
    ...
    static public void LoadFonts(string[] fonts, bool isOSFont)
    {
        for (int i = 0; i < fonts.Length; ++i)
        {
            Font font = null;
            if (isOSFont)
                font = Font.CreateDynamicFontFromOSFont(fonts[i], 16);
            else
            {
                font = Resources.Load<Font>(fonts[i]);
                Resources.UnloadUnusedAssets();
            }

            if (font != null)
            {
                mFontDictionary.Add(font.fontNames[0], font);
            }
        }
    }
    ...
    {
        Font font = null;
        mFontDictionary.TryGetValue(fontName, out font);
        return font;
    }
	...
	static public bool HasLanguage (string languageName)
	{
        if (!localizationHasBeenSet) LoadDictionary(PlayerPrefs.GetString("Language", "English"));
	...
	static bool SelectLanguage (string language)
	{
		...
		for (int i = 0, imax = mLanguages.Length; i < imax; ++i)
		{
			if (mLanguages[i] == language)
			{
				...
				DoozyUI.UIManager.GetUiContainer.BroadcastMessage("OnLocalize", SendMessageOptions.DontRequireReceiver);
				return true;
			}
		}
		return false;
