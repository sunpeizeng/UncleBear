...
public class ComponentSelector : ScriptableWizard
{
	...
	static public void Draw<T> (string buttonName, T obj, OnSelectionCallback cb, bool editButton, params GUILayoutOption[] options) where T : Object
	{
		...
		if (show) Show<T>(cb, GetName(typeof(T)) + " Selector");
		else if (cb != null) cb(o);
	}
	...
	static public void Show<T> (OnSelectionCallback cb, string title) where T : Object { Show<T>(cb, title, new string[] {".prefab"}); }

	/// <summary>
	/// Show the selection wizard.
	/// </summary>

	static public void Show<T> (OnSelectionCallback cb, string title, string[] extensions) where T : Object
	{
