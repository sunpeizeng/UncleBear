...
namespace Lean
{
	...
	public class LeanPool : MonoBehaviour
	{
        ...
        //Kevin.Zhang, 1/23/2017
        public static LeanPool GetOrCreateInstance(GameObject prefab, bool supportName = false)
        {
            LeanPool pool = supportName ? AllPools.Find(p => (p.Prefab == prefab) || (p.Prefab != null && p.Prefab.name == prefab.name)) : AllPools.Find(p => p.Prefab == prefab);
            if (pool == null)
            {
        ...
        //Kevin.Zhang, 2/3/2017
        /// <summary>
        /// make sure the amount of clones is at least "count"
        /// </summary>
        /// <param name="count">how many clones should be made</param>
		...
		public static T Spawn<T>(T prefab, Vector3 position, Quaternion rotation, Transform parent, bool supportName = false)
			where T : Component
		{
        ...
        //加入一个bool变量,支持不是一个gameobject但名字相同也可取出来使用,而不用新建一个池
		public static GameObject Spawn(GameObject prefab, Vector3 position, Quaternion rotation, Transform parent, bool supportName = false)
		{
			if (prefab != null)
			{
                ...
                //Kevin.Zhang, 1/23/2017
                LeanPool pool = GetOrCreateInstance(prefab, supportName);
			...
			for (var i = delayedDestructions.Count - 1; i >= 0; i--)
			{
				...
				if (markedObject.Clone != null)
				{
                    ...
                    if (TimeScaleIndependent)
                        markedObject.Life -= Time.unscaledDeltaTime;
                    else
