...
namespace ShatterToolkit
{
    ...
    public class ShatterTool : MonoBehaviour
    {
        ...
        /// </param>
        public List<GameObject> Split(Plane[] planes)
        {
            if (planes != null && planes.Length > 0 && hull != null && !hull.IsEmpty)
            {
                ...
                Destroy(gameObject);
                return new List<GameObject>(newGameObjects);
            }
            return null;
        }
        ...
        protected void CreateNewGameObjects(IList<IHull> newHulls, out GameObject[] newGameObjects)
        {
            ...
            for (int i = 0; i < newHulls.Count; i++)
            {
                ...
                //Set box collider
                BoxCollider newBoxCollider = newGameObject.GetComponent<BoxCollider>();
                if (newBoxCollider != null)
                {
                    GameObject.DestroyImmediate(newBoxCollider);
                ...
                if (GetComponent<Rigidbody>() != null && newRigidbody != null)
                {
                    newRigidbody.isKinematic = false;
                    newRigidbody.mass = GetComponent<Rigidbody>().mass;//GetComponent<Rigidbody>().mass * (volume / totalVolume);
